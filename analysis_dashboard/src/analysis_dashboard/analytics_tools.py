...
def calculate_token_trends(logs_json: str) -> str:
    ...
    for entry in entries:
        ...
        # ):
        #     continue
        # Also check for estimated tokens if no direct count
        ...
        # Accept entries with token data regardless of interaction type
        # But log a warning if it's not a sampling_request
        if token_count is None:
            # Try content-based estimation as last resort
            if entry.get("mcp_interaction_type") == "sampling_request":
                content = entry.get("content", "")
                if content:
                    token_count = len(content) // 4  # Very rough estimate
                    logger.info("Estimated token_count from content: %d", token_count)
...
# matplotlib.use("Agg")  # non-interactive backend - safe in both agent & Streamlit
...
# def _parse_log_list(logs_json: str) -> list[dict[str, Any]]:
#     """Parse JSON string from retrieval tools into a list of log dicts."""
#     try:
#         data = json.loads(logs_json)
#         if isinstance(data, list):
#             return data
...
#             {
#                 "status": "no_data",
#                 "message": "No latency_ms values found in provided logs.",
#             }
#         )
...
#     token_points: list[dict[str, Any]] = []
#     for entry in entries:
#         if entry.get("mcp_interaction_type") not in ("sampling_request",):
#             continue
#         metadata = entry.get("metadata", {})
#         token_count = metadata.get("token_count") or metadata.get("response_length")
...
#             {
#                 "status": "no_data",
#                 "message": "No token_count values found in sampling_request entries.",
#             }
#         )
...
#     Identifies entries with component paths containing "error" or
#     metadata keys indicating failures, then groups them into time
#     buckets to produce frequency data.
...
#                 "status": "ok",
#                 "total_errors": 0,
#                 "message": "No error entries found in the provided logs.",
#             }
#         )
...
#         metric_json: JSON string from one of the metric calculation tools.
#         chart_type:  One of "latency", "tokens", "errors", "combined".
#         save_to_disk: If True, save the chart as a PNG file to the
#                      charts output directory.
#         filename:    Optional filename (without extension). Defaults to
#                      "<chart_type>_<timestamp>.png".

#     Returns:
#         JSON with "chart_path" (if saved), "chart_type", and a
#         "description" of what was plotted.  Also returns the
#         figure as a base64-encoded PNG for inline display.
#     """
#     logger.info(
#         "generate_performance_chart | chart_type=%s | save=%s",
# chart_type, save_to_disk
#     )
#     import base64
#     from io import BytesIO
...
#         # ── Error frequency chart ─────────────────────────────────
#         elif chart_type == "errors":
#             # error_entries = data.get("error_entries", [])
#             per_session = data.get("per_session_counts", {})
#             if per_session:
#                 sessions = [s[:8] + "…" for s in per_session.keys()]
#                 counts = list(per_session.values())
...
#                     sessions, counts, color="#f38ba8", alpha=0.85, edgecolor="#313244"
#                 )
#                 ax.set_title("Error Frequency per Session",
...
#                 for spine in ax.spines.values():
#                     spine.set_edgecolor("#45475a")
#                 description = f"Error chart: {data.get('total_errors', 0)}
# total errors"
#             else:
#                 ax.text(
#                     0.5,
...
#         # Encode to base64 for Streamlit inline display
#         buf = BytesIO()
#         plt.savefig(
#             buf, format="png", bbox_inches="tight", facecolor=fig.get_facecolor()
#         )
...
#             charts_dir = _ensure_charts_dir()
#             ts = datetime.now().strftime("%Y%m%d_%H%M%S")
#             fname = (filename or f"{chart_type}_{ts}") + ".png"
#             chart_path = os.path.join(charts_dir, fname)
#             fig.savefig(
#                 chart_path, bbox_inches="tight", dpi=120,
...
#             | {"has_image": True, "image_base64": img_b64},
#             indent=2,
#         )
